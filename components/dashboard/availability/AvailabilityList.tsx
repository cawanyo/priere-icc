...
export function AvailabilityList({ items }: AvailabilityListProps) {
  ...
  return (
    <Card className="border-none shadow-md h-full">
      <CardHeader>
        <CardTitle className="text-lg font-serif text-indigo-900">Mes Absences à venir</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.map((item) => {
            const isRange = item.startDate.getTime() !== item.endDate.getTime();
            return (
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            <span className="font-medium text-gray-900">
                                ...
                                {isRange && ` - ${format(new Date(item.endDate), "d MMM yyyy", { locale: fr })}`}
                                {!isRange && format(new Date(item.startDate), " yyyy", { locale: fr })}
                            </span>
                        </div>
